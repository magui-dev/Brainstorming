...
public class BrainstormingService {
    ...
    private static final String PYTHON_BASE_URL = "http://localhost:8000/api/v1/brainstorming";
    ...
    public BrainstormResponse generate(BrainstormRequest request) {
        try {
            ...
            // 7. DB 저장
            List<IdeaResponseDto> savedIdeas = saveIdeasToDb(request.getUserId(), ideasResponse);
    ...
    private SessionResponse createSession() {
        String url = PYTHON_BASE_URL + "/session";
    ...
    private PurposeResponse submitPurpose(String sessionId, String purpose) {
        String url = PYTHON_BASE_URL + "/purpose";
    ...
    private WarmupResponse getWarmupQuestions(String sessionId) {
        String url = PYTHON_BASE_URL + "/warmup/" + sessionId;
    ...
    private ConfirmResponse confirmWarmup(String sessionId) {
        String url = PYTHON_BASE_URL + "/confirm/" + sessionId;
    ...
    private AssociationsResponse submitAssociations(String sessionId, List<String> associations) {
        String url = PYTHON_BASE_URL + "/associations/" + sessionId;
    ...
    private IdeasResponse generateIdeas(String sessionId) {
        String url = PYTHON_BASE_URL + "/ideas/" + sessionId;
    ...
    private void deleteSession(String sessionId) {
        String url = PYTHON_BASE_URL + "/session/" + sessionId;
     ...
     */
    private List<IdeaResponseDto> saveIdeasToDb(Long userId, IdeasResponse ideasResponse) {
